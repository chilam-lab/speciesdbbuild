...
try:
    ...
    def flush_pg_notices(db_conn, prefix='PG NOTICE'):
        if not getattr(db_conn, 'notices', None):
            return
        for notice in db_conn.notices:
            logger.info(f'{prefix}: {notice.strip()}')
        db_conn.notices.clear()

    logger.info('Configurando FDW hacia meshandregions_db')
    mesh_fdw_sql = get_sql(create_mesh_fdw_sql)
    mesh_fdw_sql = mesh_fdw_sql.replace('__MESHDB_HOST__', DBMESHHOST)
    mesh_fdw_sql = mesh_fdw_sql.replace('__MESHDB_PORT__', DBMESHPORT)
    mesh_fdw_sql = mesh_fdw_sql.replace('__MESHDB_NAME__', DBMESHNAME)
    mesh_fdw_sql = mesh_fdw_sql.replace('__MESHDB_USER__', DBMESHUSER)
    mesh_fdw_sql = mesh_fdw_sql.replace('__MESHDB_PASS__', DBMESHPASSWD.replace("'", "''"))
    ...
    cur.execute(create_cat_taxon_sql)
    flush_pg_notices(conn, 'CAT_TAXON')
    logger.info('SQL de cat_taxon ejecutado')
    ...
    cur.execute("""
        ...
        min(level_size),
        max(level_size),
        max(array_length(available_grids, 1))
        FROM cat_taxon;
    """)
    row = cur.fetchone()
    logger.info(
        f'cat_taxon verificacion -> rows={row[0]}, min_level_size={row[1]}, '
        f'max_level_size={row[2]}, grids_count={row[3]}'
    )

    cur.close()
    conn.close()
    logger.info('cat_taxon creado correctamente')

except Exception as err:
    logger.error('No se creó correctamente cat_taxon: {0}'.format(str(err)))
    err_type, err_obj, traceback = sys.exc_info()
    line_num = traceback.tb_lineno
    print("\nERROR:", err, "on line number:", line_num)
    print("traceback:", traceback, "-- type:", err_type)
