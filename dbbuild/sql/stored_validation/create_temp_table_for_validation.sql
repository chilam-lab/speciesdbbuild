CREATE OR REPLACE FUNCTION public.createtemptableforvalidation(spid integer, tblname text, iter integer, res_celda_sp text, res_celda_snib text, res_celda_snib_tb text)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$  
declare
...
begin
		...
		execute format('
			...
			(
				select spid, cells 
				from (
					select spid, unnest(%s) as cells
					from sp_snib
					where sp_snib.spid = %s
				) as table_a
				order by random()
			) as tab_a
			group by spid', res_celda_sp, spid)
